...
const BlogsCard = ({ blog }: BlogsCardProps) => {
  return (
    <div className="border rounded-lg  p-1">
      ...
      <h1 className="text-lg md:text-xl cursor-pointer font-semibold hover:text-secondary transition-all">
        {blog.title}
      </h1>
      <p className="py-2 text-base leading-6 line-clamp-2">{blog.desc}</p>
      <div className="flex items-center gap-2 pt-2">
        <img src={blog.user} alt="User" className="w-6 object-cover h-6 rounded-full" />
